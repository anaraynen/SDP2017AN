...
object ScalaBasics {
  ...
  /**
    ...
    */
  def inRange(start: Int, end: Int): Range = Range.inclusive(start, end)
  ...
  /**
    ...
    */
  def oddRange(n: Int): Range = Range.inclusive(1, n, 2)
  ...
  def minRecursive(r: Array[Int]): Int = {
    ...
    if(r.length == 1){
      ...
      minRecursive(r.head, minRecursive(r.tail))
    }

  }

  def minRecursive(x: Int, y: Int): Int = if (x < y) x else y
  ...
  def isPalindrome(s: String): Boolean = {
    ...
    val sReplaced = s.replaceAll("[^a-zA-Z]+", " ")
    val sSpaces = sReplaced.replaceAll(" ", "")
    val sNew = sSpaces.toLowerCase

    val str = for {i <- 2 to sNew.size; str <- sNew.sliding(i) if sNew == str.reverse} yield true
