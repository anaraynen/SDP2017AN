...
object Funcs {
  ...
  def drop[A](ls: List[A], n: Int): List[A] = ls match {
    ...
    case head :: tail if n > 1 => drop(tail, n-1)
  ...
  def reverse[A](ls: List[A]): List[A] = foldLeft(ls, List[A]())((head, tail) => tail::head)

  def flatten[A](ls: List[List[A]]): List[A] = foldLeft(ls, List[A]())((head, tail) => head ::: tail)
